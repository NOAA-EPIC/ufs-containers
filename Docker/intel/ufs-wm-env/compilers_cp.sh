...
if [[ ${ARGC} -lt 2 &&  ${ARGC} -gt 0 ]]; then
  ...
  die "Usage incorrect, provide two arguments exactly, or no arguments to use default names"
else
  intel_sandbox=${1:-intel-sandbox}
  target_sandbox=${2:-rocky9-oneapi2024.2-ss192}
  echo "INTEL-SANDBOX is $intel_sandbox "
  echo "TARGET-SANDBOX is $target_sandbox "
fi  

cmd=${CMD:-cp -pr}

intel_sandbox=${1:-intel-sandbox}
target_sandbox=${2:-rocky9-oneapi2024.2_nc-ss192}
