...
cp Dockerfile.ubuntu20.04-intel-landda Dockerfile.ubuntu20.04-intel-stage2
sed -i "s/TAGNAME/$tag_name/g" Dockerfile.ubuntu20.04-intel-stage2
docker build -f Dockerfile.ubuntu20.04-intel-stage2 -t noaaepic/ubuntu20.04-intel-landda:$tag_name --build-arg branch_name=$branch_name . 
...
docker run -d --name spack-stack noaaepic/ubuntu20.04-intel-landda:$tag_name
