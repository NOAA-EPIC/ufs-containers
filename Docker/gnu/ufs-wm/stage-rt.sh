...
[ ! -d $PWD/ufs-weather-model/bin ] && mkdir -p $PWD/ufs-weather-model/bin
[ ! -d $PWD/ufs-weather-model/container-scripts ] && mkdir -p $PWD/ufs-weather-model/container-scripts
...
ln -s ../container-scripts/run_container_executable.sh python3
cd ../..
...
sed -i "s|USER_COMPILER|$compiler|g" $PWD/ufs-weather-model/tests/fv3_conf/fv3_slurm.IN_singularity
sed -i "s|USER_MPI|$mpi|g" $PWD/ufs-weather-model/tests/fv3_conf/fv3_slurm.IN_singularity
...
cp $PWD/ufs-weather-model/modulefiles/ufs_singularity.intel.lua $PWD/ufs-weather-model/tests-dev/modules.fv3_atm_gnu.lua
...
# Trick rt.sh file
sed -i "571 i export MACHINE_ID=singularity" $PWD/ufs-weather-model/tests/rt.sh
# Add singularity to gnu RTs
sed -i 's|ursa|ursa singularity|g' $PWD/ufs-weather-model/tests/rt.conf
...
sed -i "s|SINGULARITY_WORKING_DIR|$PWD|g" $PWD/ufs-weather-model/container-scripts/ufs-wm.env
sed -i "s|SINGULARITY_WORKING_DIR|$PWD|g" $PWD/ufs-weather-model/container-scripts/*_executable.sh
sed -i "s|SINGULARITY_WORKING_DIR|$PWD|g" $PWD/ufs-weather-model/tests/rt.sh
