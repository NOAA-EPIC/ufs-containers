...
case ${MACHINE_ID} in
  ...
  frontera)
    ...
    ;;
  singularity)
    echo "rt.sh: Setting up singularity..."
    export PATH="SINGULARITY_WORKING_DIR/ufs-weather-model/bin:${PATH}"
    # Modify based on YOUR platform
    #module use /apps/modules/modulefiles
    #if [[ "${ROCOTO:-false}" == true ]] ; then
    #  module load rocoto/1.3.7
    #  ROCOTO_SCHEDULER=slurm
    ...
    COMPILE_QUEUE="batch"
    PARTITION=
    dprefix=SINGULARITY_WORKING_DIR
    # Update to location on YOUR platform
    DISKNM="/contrib/ufs-weather-model/RT"
    STMP=SINGULARITY_WORKING_DIR
    PTMP=SINGULARITY_WORKING_DIR
    SCHEDULER="slurm"
    ;;
  *)
